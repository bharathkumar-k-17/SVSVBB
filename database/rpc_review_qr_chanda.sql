-- Migration to create Secure RPCs for QR Chanda Review
-- Drop existing functions to avoid signature conflicts
DROP FUNCTION IF EXISTS review_qr_chanda_accept(UUID);
DROP FUNCTION IF EXISTS review_qr_chanda_reject(UUID, TEXT);
DROP FUNCTION IF EXISTS review_qr_chanda_reject(TEXT, UUID);
...
BEGIN
    ...
    RETURN UPPER(v_role) IN ('ADMIN', 'SUPER_ADMIN', 'SUPERADMIN');
END;
$$;
...
BEGIN
    ...
    INSERT INTO notifications (
        ...
        'QR CHANDA REVIEW',
        'QR Chanda Request from ' || v_request.name || ' for ₹' || COALESCE(v_request.paid_amount, 0) || ' was ACCEPTED by ' || v_user.name || '. Receipt: ' || v_receipt_no,
        COALESCE(v_request.paid_amount, 0),
        v_now,
        v_user.id,
        v_user.name,
...
-- Reload Supabase Schema Cache mappings
