...
export function ChandaEntry() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.paymentMode === 'Cash' && !paidToUserId) {
                ...
                {todaysCollections
                  .map((dev: any, idx: number) => (
                    <tr key={dev.id} className="hover:bg-white/40 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center">
                        <span className={`inline-block text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider ${dev.paymentStatus === 'PAID' ? 'bg-emerald-100 text-emerald-800' :
